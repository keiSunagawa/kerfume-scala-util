...
trait NelSupport {
  protected def whenNel0[A, B, F[_]: MonoidK, G[_]: Applicative](
    ...
    NonEmptyList.fromList(xs) match {
      case Some(nel) => f(nel)
      case None      => implicitly[Applicative[G]].pure(implicitly[MonoidK[F]].empty[B])
    }
  }
  ...
  implicit class ListOps[A](xs: List[A]) {
    def whenNel[B, F[_]: MonoidK, G[_]: Applicative](f: NonEmptyList[A] => G[F[B]]): G[F[B]] = whenNel0(xs)(f)
    def tapWhenNel[G[_]: Applicative](f: NonEmptyList[A] => G[Unit]): G[Unit] = tapWhenNel0(xs)(f)
  }
}
